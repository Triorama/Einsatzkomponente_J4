...
if ($this->params->get('display_einsatzkarte_einsatzgebiet', '1')):
  $catbox .=
    '<div class="eiko_gmap_toolbar"><label for="area"><button type="button" class="btn btn-default btn-xs eiko_gmap_toolbar_button" onClick="togglearea()" id="div_area"><input type="checkbox" class="eiko_gmap_checkbox" id="area" onClick="togglearea()" checked/>&nbsp;&nbsp;<img src="' .
    JURI::base() .
    'images/com_einsatzkomponente/images/map/icons/' .
    $this->params->get('einsatzkarte_orga_image', 'haus_rot.png') .
    '" class="eiko_gmap_toolbar_icon" />Einsatzgebiet anzeigen</button></label></div>';
endif;
if (!$this->params->get('display_einsatzkarte_einsatzgebiet', '1')):
  $catbox .= '';
endif;
...
); ?>"+id+">zur Detailansicht</a><?php endif; ?></div>";
...
); ?>" + "&view=show&Itemid=" + itemid + "&gmaplink=1&id=" + id + "> zum Detailbericht <\/a></p></div></td><td style='padding-right:20px;margin-right:20px;'><img style='border:1px solid;' src='/components/com_reports2/images/noimage.png' height='90' /></td></tr></table>";
...
<button type="button" id="einsatzarten_flip" class="btn btn-sm">Einsatzarten auswählen</button>
    ...
    '</div>'; ?>
		...
		<div class="well eiko_gmap_sidebar span6" id="map" style="height:<?php echo $this->params->get(
    'einsatzkarte_map_height',
    '450'
  ); ?>px;width:98%;margin-left:0px;"></div>
		<?php endif; ?>
		<?php if (!$this->params->get('display_einsatzkarte_map', '1')): ?>
		<div class="well eiko_gmap_sidebar span6" id="map" style="height:<?php echo $this->params->get(
    'einsatzkarte_map_height',
