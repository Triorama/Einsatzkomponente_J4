        ...
        <div class="row">
			...
			<?php endif; ?>
                ...
                <ul>
					...
					<?php echo '<br/><li>Link zur Webseite: <a href="" target="blank" class="eiko_fahrzeuge_link">' .
       $this->item->link .
       '</a></li>'; ?>
					<?php endif; ?>
					<?php endif; ?>
...
<?php if ($this->params->get('show_fahrzeuge_ausruestung', '0')):
  if (!$this->item->ausruestung == ''):
    if ($this->params->get('eiko', '0')):
      ...
      endforeach;
      ?>
 <br/><li><?php echo Text::_(
   'COM_EINSATZKOMPONENTE_BELADUNG_AUSRUESTUNG'
 ); ?>: <ul><?php echo $ausruestung; ?></ul></li> 
 <?php
    endif; ?>
 <?php
  endif; ?>
...
<input type="button" class="btn eiko_back_button" value="<?php echo Text::_(
  'COM_EINSATZKOMPONENTE_ZURUECK'
    ...
    'index.php?option=com_einsatzkomponente&view=einsatzfahrzeugform.&id=' . $this->item->id
  ); ?>"><?php echo Text::_('COM_EINSATZKOMPONENTE_EDIT'); ?></a>
	<?php endif; ?>
								<?php
  //if(JFactory::getUser()->authorise('core.delete','com_einsatzkomponente.einsatzfahrzeug.'.$this->item->id)):
  ?>
									<!-- <a class="btn" href="<?php echo Route::_(
