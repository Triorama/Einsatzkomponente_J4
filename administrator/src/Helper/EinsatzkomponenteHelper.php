...
class EinsatzkomponenteHelper extends ComponentHelper
{
  /**
...
google.maps.event.addListener(map, "click", function() { infowindow_marker2.close(); });
  ...
  var image = "' .
    ...
    scaledSize: new google.maps.Size(' .
      $params->get('einsatzkarte_gmap_icon', 14) .
      ',' .
      $params->get('einsatzkarte_gmap_icon', 14) .
      ') 
    };
...
var request = {
      ...
      $standort->gmap_latitude .
      '","' .
      $standort->gmap_longitude .
      '"),
...
function setMarkers(map, locations) {
  for (var i = 0; i < locations.length; i++) {
    ...
    url: "' .
      ...
      '"+orgas[4],
    scaledSize: new google.maps.Size(' .
      $params->get('einsatzkarte_gmap_icon_orga', 24) .
      ',' .
      $params->get('einsatzkarte_gmap_icon_orga', 24) .
      ') 
    };
...
function setMarkerz(map, locations) {
  for (var i = 0; i < locations.length; i++) {
    ...
    url: "' .
      ...
      '"+einsatzorte[4],
    scaledSize: new google.maps.Size(' .
      $params->get('einsatzkarte_gmap_icon', 8) .
      ',' .
      $params->get('einsatzkarte_gmap_icon', 8) .
      ') 
    };
...
function drawmap() {
	...
	map = new OpenLayers.Map ("map", {minZoom:1, maxZoom:' .
      $gmap_zoom_level .
      '});
...
icons[4] = new Array("' .
      ...
      $marker1_image .
      '","' .
      $params->get('einsatzkarte_gmap_icon', 24) .
      '","' .
      $params->get('einsatzkarte_gmap_icon', 24) .
      '","0","1");
...
geometries = new Array();geometries.push(drawPolygon(' .
      $einsatzgebiet .
  ...
  public static function kalender($value, $name, $id, $format = '%Y-%m-%d', $attribs = null)
  {
    ...
    if (!in_array($id, $done)) {
      ...
      $document->addScriptDeclaration(
		...
		singleClick: true,
		firstDay: ' .
          Factory::getLanguage()->getFirstDay() .
          '
		});});'
      );
      $done[] = $id;
    ...
    return '<div' .
      ...
      (0 !== (int) $value ? HTMLHelper::_('date', $value, null, null) : '') .
      '" name="' .
      $name .
      '" id="' .
      $id .
      '" value="' .
      htmlspecialchars($inputvalue, ENT_COMPAT, 'UTF-8') .
      '" ' .
      $attribs .
      ' />' .
  ...
  public static function getEinsatzdauer($date1, $date3)
  {
    ...
    if ($diff < 60):
      if ($diff == 0):
        ...
        $einsatzdauer = $diff . ' Min.';
      endif; // Anzahl Tage = Sekunden /24/60/60
      ...
      if ($days > 0):
        $einsatzdauer = $days . ' Tag(e) ' . $hours . ' Std. ' . $minutes . ' Min.';
      else:
        $einsatzdauer = $hours . ' Std. ' . $minutes . ' Min.';
      endif;
    endif;
